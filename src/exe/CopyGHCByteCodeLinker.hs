...
resolvePtr interp le bco_ix breakarray ptr = case ptr of
  BCOPtrName nm
    | Just ix <- {-# SCC lookupNameEnv1 #-} lookupNameEnv bco_ix nm
    -> return (ResolvedBCORef ix) -- ref to another BCO in this group

    | Just (_, rhv) <- {-# SCC lookupNameEnv2 #-} lookupNameEnv (closure_env le) nm
    ...
    -> assertPpr (isExternalName nm) (ppr nm) $
       do
          ...
          m <- {-# SCC lookupSymbol #-} lookupSymbol interp sym_to_find
          case m of
            Just p -> return (ResolvedBCOStaticPtr (toRemotePtr p))
            Nothing -> linkFail "GHC.ByteCode.Linker.lookupCE" (unpackFS sym_to_find)

  BCOPtrPrimOp op
    -> ResolvedBCOStaticPtr <$> {-# SCC lookupPrimOp #-} lookupPrimOp interp op

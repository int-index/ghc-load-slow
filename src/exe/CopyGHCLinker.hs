...
linkSomeBCOs bco_opts interp le mods = foldr fun do_link mods []
 where
  ...
  do_link mods = do
    ...
    resolved <- {-# SCC do_link_sequence_kana_sama_style #-}
                sequence [ {-# SCC linkBCO #-} linkBCO interp le bco_ix breakarray bco
                         | (breakarray, bco) <- flat ]
    hvrefs <- {-# SCC do_link_createBCOs #-} createBCOs interp bco_opts resolved
